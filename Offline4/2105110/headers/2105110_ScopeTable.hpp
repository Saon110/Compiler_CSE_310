...
public:
    ...
    bool insert(SymbolInfo &symbol, ostream &out = cout)
    {
        ...
        SymbolInfo *new_symbol = new SymbolInfo(symbol.getName(), symbol.getType(), symbol.getIsArray(), symbol.getIsGlobal(), symbol.getIsFunction(), symbol.getIsParameter(), symbol.getOffset());
        int i = 0;
        ...
        else
        {
            ...
            while (temp != nullptr)
            {
                if (temp->getName() == symbol.getName())
                {
                    ...
                    symbol.print(out);
                    out << " already exists in ScopeTable# " << display_id << " at position " << index << ", " << i << endl
                        << endl;
